...
if action == 'discover':
    ...
    nb = json.loads((ROOT / 'artifacts/gold/nb_gold_bookings_by_city.ipynb').read_text())
    for c in nb['cells']:
        if c['cell_type'] == 'code':
...
if action in ('deploy', 'readback'):
    ...
    original = json.loads((ROOT / 'artifacts/gold/nb_gold_bookings_by_city.ipynb').read_text())
